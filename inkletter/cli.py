...
@theme_options
def preview(filepath: Path, output: Path | None, theme_name: str | None, no_theme: bool):
    """
...
@theme_options
def md2mjml(filepath: Path, output: Path | None, theme_name: str | None, no_theme: bool):
...
@theme_options
def md2html(filepath: Path, output: Path | None, view: bool, theme_name: str | None, no_theme: bool):
    """Convert Markdown to HTML, optionally save and open it."""
    theme = load_theme(theme_name, no_theme)
    try:
        markdown_text = filepath.read_text(encoding="utf-8")
        html_output = parse_markdown_to_html(markdown_text, theme=theme)
    except Exception as e:
        raise click.ClickException(str(e))

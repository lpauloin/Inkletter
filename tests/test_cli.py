...
DJANGO_SOURCE = "Hi {{ customer.name }},\n\n**[Track]({% url 'delivery' %})**\n"


def test_md2html_django_flag(tmp_path):
    md = tmp_path / "in.md"
    md.write_text(DJANGO_SOURCE, encoding="utf-8")
    out = tmp_path / "out.html"

    result = CliRunner().invoke(cli, ["md2html", str(md), "--django", "-o", str(out)])

    assert result.exit_code == 0
    content = out.read_text(encoding="utf-8")
    assert "{{ customer.name }}" in content
    assert "href=\"{% url 'delivery' %}\"" in content


def test_md2txt_django_flag(tmp_path):
    md = tmp_path / "in.md"
    md.write_text(DJANGO_SOURCE, encoding="utf-8")
...
def test_md2mjml_django_flag(tmp_path):
    ...
    result = CliRunner().invoke(cli, ["md2mjml", str(md), "--django"])

    assert result.exit_code == 0
    assert "{{ customer.name }}" in result.output
    assert "inktag" not in result.output


def test_preview_django_flag_survives_a_lower_than(tmp_path, monkeypatch):
    # the preview compiles the MJML itself: if it fed the compiler the
    # restored tags, the < would make it raise
    monkeypatch.setattr(cli_module.webbrowser, "open", lambda url: None)
    md = tmp_path / "in.md"
    md.write_text("{% if a < b %}yes{% endif %}\n", encoding="utf-8")
    out = tmp_path / "preview.html"

    result = CliRunner().invoke(cli, ["preview", str(md), "--django", "-o", str(out)])

    assert result.exit_code == 0
    # the preview page embeds the email in a JS string, so its < comes
    # out as < — the tag itself is what matters here
    assert "{% endif %}" in out.read_text(encoding="utf-8")


def test_django_flag_is_off_by_default(tmp_path):
    md = tmp_path / "in.md"
    md.write_text("[a]({{ url }})\n", encoding="utf-8")
